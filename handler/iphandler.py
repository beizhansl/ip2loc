...
class IPHandler:
    def __init__(self, filename=None):
        self.filename = filename if filename else '/home/ip2loc/handler/GeoLite2-City.mmdb'
        self.reader = None
        self.xdbsearcher = None
        self.loc = None
        self.init_reader()
        self.init_xdbsearcher()
    ...
    def init_reader(self):
        ...
        while attempts < max_attempts:
            try:
                ...
                self.close_reader()        
                logging.error(f"open geoip2 reader error, {str(e)}")
                attempts += 1
                time.sleep(1)
                self.reader = None
    
    def init_xdbsearcher(self):
        dbPath = "/home/ip2loc/handler/ip2region.xdb"
        attempts = 0
        max_attempts = 3
        while attempts < max_attempts:
            try:
                ...
                with open('/home/ip2loc/handler/loc.json', 'r', encoding='utf-8') as f:
        ...
        for ip_str, count in ip_dict.items():
            try:
                ...
                if res is None or res.location.latitude is None or res.location.longitude is None or (res.location.latitude == 34.7732 and res.location.longitude == 113.722):
                    region_str = self.xdbsearcher.searchByIPStr(ip_str)
                ...
                logging.warning(f"translate ip to loc error. {str(e)}")
